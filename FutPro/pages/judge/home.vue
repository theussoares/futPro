...
const fetchMatches = async () => {
  const snapshot = await getDocs(
    query(collection(db, 'matches'), where('judges', 'array-contains', auth.currentUser?.uid))
  );
...
const goToStartMatch = (matchId: string) => {
  // Redirecionar para a página de iniciar partida
  router.push(`/judge/start-match?id=${matchId}`);
};
